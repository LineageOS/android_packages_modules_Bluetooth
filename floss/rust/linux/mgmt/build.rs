...
fn gen_mod_rs<P: AsRef<Path>>(out_dir: PathBuf, inputs: &[P]) {
    ...
    for i in inputs {
        let stem = i.as_ref().file_stem().unwrap();
...
fn main() {
    ...
    if let Err(e) = fs::create_dir(proto_out_dir.as_os_str().to_str().unwrap()) {
        println!("Proto dir failed to be created: {}", e);
    }

    protobuf_codegen::Codegen::new()
        .out_dir(proto_out_dir.as_os_str().to_str().unwrap())
        .inputs(&proto_input_files)
        .includes(&proto_include_dirs)
