...
class BtmanagerdTest(unittest.TestCase):
    ...
    def _register_hci_device_change_observer(self):
        return self.manager_object.RegisterStateChangeObserver(
            self.client_object_path, dbus_interface=MANAGER_INTERFACE_NAME)

    def _unregister_hci_device_change_observer(self):
        return self.manager_object.UnregisterHciDeviceChangeObserver(
            self.client_object_path, dbus_interface=MANAGER_INTERFACE_NAME)
    ...
    def _register_hci_device_change_observer(self):
        return self.manager_object.RegisterHciDeviceChangeObserver(
            self.client_object_path, dbus_interface=MANAGER_INTERFACE_NAME)
