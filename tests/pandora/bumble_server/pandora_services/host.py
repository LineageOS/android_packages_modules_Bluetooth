...
class HostService(HostServicer):
    ...
    @utils.rpc
    async def FactoryReset(self, request: empty_pb2.Empty,
    ...
    @utils.rpc
    async def Reset(self, request: empty_pb2.Empty,
                    context: grpc.ServicerContext) -> empty_pb2.Empty:
    ...
    async def ReadLocalAddress(self, request: empty_pb2.Empty,
                               context: grpc.ServicerContext) -> ReadLocalAddressResponse:
    ...
    async def Connect(self, request: ConnectRequest,
                      context: grpc.ServicerContext) -> ConnectResponse:
    ...
    @utils.rpc
    async def WaitConnection(self, request: WaitConnectionRequest,
                             context: grpc.ServicerContext) -> WaitConnectionResponse:
        ...
        self.log.debug(f"WaitConnection from {address} done (handle={connection.handle})")

        cookie = any_pb2.Any(value=connection.handle.to_bytes(4, 'big'))
        return WaitConnectionResponse(connection=Connection(cookie=cookie))

    @utils.rpc
    async def ConnectLE(self, request: ConnectLERequest,
                        context: grpc.ServicerContext) -> ConnectLEResponse:
    ...
    @utils.rpc
    async def Disconnect(self, request: DisconnectRequest,
                         context: grpc.ServicerContext) -> empty_pb2.Empty:
    ...
    @utils.rpc
    async def WaitDisconnection(self, request: WaitDisconnectionRequest,
                                context: grpc.ServicerContext) -> empty_pb2.Empty:
        connection_handle = int.from_bytes(request.connection.cookie.value, 'big')
    ...
    @utils.rpc
    async def Advertise(self, request: AdvertiseRequest,
                        context: grpc.ServicerContext) -> AsyncGenerator[AdvertiseResponse, None]:
        try:
            if request.legacy:
    ...
    async def extended_advertise(
        ...
        advertising_parameters = AdvertisingParameters(
            ...
            secondary_advertising_phy=SECONDARY_PHY_TO_BUMBLE_PHY_MAP[request.secondary_phy],
        )
        if advertising_interval := request.interval:
            advertising_parameters.primary_advertising_interval_min = int(advertising_interval)
            advertising_parameters.primary_advertising_interval_max = int(advertising_interval)
    ...
    async def legacy_advertise(
        ...
        if data := request.data:
            ...
            for service in self.device.gatt_server.attributes:
                if isinstance(service, Service) and (service_data :=
                                                     service.get_advertising_data()):
                    service_uuid = service.uuid.to_hex_str('-')
                    if (service_uuid in request.data.incomplete_service_class_uuids16 or
                    ...
                    if (service_uuid in scan_response_data.incomplete_service_class_uuids16 or
    ...
    async def Scan(self, request: ScanRequest,
                   context: grpc.ServicerContext) -> AsyncGenerator[ScanningResponse, None]:
    ...
    async def Inquiry(self, request: empty_pb2.Empty,
                      context: grpc.ServicerContext) -> AsyncGenerator[InquiryResponse, None]:
    ...
    @utils.rpc
    async def SetDiscoverabilityMode(self, request: SetDiscoverabilityModeRequest,
                                     context: grpc.ServicerContext) -> empty_pb2.Empty:
    ...
    @utils.rpc
    async def SetConnectabilityMode(self, request: SetConnectabilityModeRequest,
                                    context: grpc.ServicerContext) -> empty_pb2.Empty:
                ...
                bytes([
                    ...
                    *struct.pack(
                        ...
                        (dt.peripheral_connection_interval_max
                         if dt.peripheral_connection_interval_max else
                         dt.peripheral_connection_interval_min),
                    ),
                ]),
            ))
    ...
    def pack_data_types(self, ad: AdvertisingData) -> DataTypes:
        ...
        if uuids := cast(
            ...
            dt.incomplete_service_class_uuids16.extend(list(map(lambda x: x.to_hex_str('-'),
                                                                uuids)))
        if uuids := cast(
                List[UUID],
        ...
        if uuids := cast(
            ...
            dt.incomplete_service_class_uuids32.extend(list(map(lambda x: x.to_hex_str('-'),
                                                                uuids)))
        if uuids := cast(
                List[UUID],
