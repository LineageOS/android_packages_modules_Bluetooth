...
public class BluetoothManagerService {
  ...
  private static final boolean DEGRADED_PERFORMANCE =
      SystemProperties.getBoolean("bluetooth.hardware.degraded_performance_mode.enabled", false);
  ...
  private final IBluetoothCallback mBluetoothCallback =
      new IBluetoothCallback.Stub() {
        ...
        public void onAdapterAddressChange(String address) {
          ...
          Log.d(TAG, "IBluetoothCallback.onAdapterAddressChange: " + Log.address(address));
        ...
        public void setAdapterServiceBinder(IBinder binder) {
          ...
          post(
              () -> {
                ...
                broadcastToAdapters(
                    "setAdapterServiceBinder", (item) -> item.onBluetoothServiceUp(binder));
              });
        }
      };
  ...
  private void storeAddress(String address) {
    if (!Settings.Secure.putString(mContentResolver, Settings.Secure.BLUETOOTH_ADDRESS, address)) {
      Log.e(
          TAG,
          "storeAddress("
              + Log.address(address)
        ...
        public void onReceive(Context context, Intent intent) {
          ...
          if (Intent.ACTION_SETTING_RESTORED.equals(action)) {
            ...
            if (Settings.Global.BLUETOOTH_ON.equals(name)) {
              ...
              final String prevValue = intent.getStringExtra(Intent.EXTRA_SETTING_PREVIOUS_VALUE);
  ...
  BluetoothManagerService(
    ...
    mConfigAllowAutoOn = SystemProperties.getBoolean("bluetooth.server.automatic_turn_on", false);
  ...
  boolean enableBle(String packageName, IBinder token) {
    ...
    if (mState.oneOf(
        State.ON, State.BLE_ON, State.TURNING_ON, State.TURNING_OFF, State.BLE_TURNING_ON)) {
  ...
  /**
   * Will call startBrEdr() if bluetooth classic should be on and will call stopBle if bluetooth BLE
   * should be off
   */
  ...
  /**
   * Inform BluetoothAdapter instances that BREDR part is down and turn off all service and stack if
   * no LE app needs it
   */
  ...
  void handleOnBootPhase(UserHandle userHandle) {
    ...
    mSharingRestriction = new SharingRestriction(mUserContext, mLooper, mBluetoothComponent, mUser);
  ...
  private void sendBluetoothServiceDownCallback() {
    broadcastToAdapters(
        "sendBluetoothServiceDownCallback", IBluetoothManagerCallback::onBluetoothServiceDown);
  }
      ...
      switch (msg.what) {
        ...
        case MESSAGE_BLUETOOTH_SERVICE_CONNECTED -> {
          ...
          if (!isBinding() && !mEnable) {
            ...
            AdapterBinder adapter = BluetoothServerProxy.getInstance().createAdapterBinder(service);
  ...
  private static void setBtHciSnoopLogMode(int mode) {
    final BluetoothProperties.snoop_log_mode_values snoopMode =
        switch (mode) {
          case BT_SNOOP_LOG_MODE_DISABLED -> BluetoothProperties.snoop_log_mode_values.DISABLED;
          case BT_SNOOP_LOG_MODE_FILTERED -> BluetoothProperties.snoop_log_mode_values.FILTERED;
