...
class BluetoothServiceBinder extends IBluetoothManager.Stub {
    ...
    BluetoothServiceBinder(Looper looper, BluetoothManagerService bms, Context ctx) {
        mService = bms;
        mContext = ctx;
    ...
    @Override
    public int getState() {
        return postFromBinder(() -> mService.getState());
    }
    ...
    public boolean onFactoryReset(AttributionSource source) {
        ...
        if (state == State.BLE_TURNING_ON
            ...
            if (!mService.waitForState(State.BLE_ON, State.ON)) {
                return false;
            }
        }
        return postFromBinder(() -> mService.onFactoryReset());
    }
