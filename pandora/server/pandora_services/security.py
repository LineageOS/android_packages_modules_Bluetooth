...
class PairingDelegate(BasePairingDelegate):
    ...
    def __init__(
        ...
        local_initiator_key_distribution: BasePairingDelegate.KeyDistribution = BasePairingDelegate.
        DEFAULT_KEY_DISTRIBUTION,
        local_responder_key_distribution: BasePairingDelegate.KeyDistribution = BasePairingDelegate.
        DEFAULT_KEY_DISTRIBUTION,
    ) -> None:
    ...
    def add_origin(self, ev: PairingEvent) -> PairingEvent:
        if not self.connection.is_incomplete:
            ...
            ev.connection.CopyFrom(
                Connection(cookie=any_pb2.Any(value=self.connection.handle.to_bytes(4, 'big'))))
        else:
    ...
    async def confirm(self, auto: bool = False) -> bool:
        self.log.debug(f"Pairing event: `just_works` (io_capability: {self.io_capability})")
    ...
    async def compare_numbers(self, number: int, digits: int = 6) -> bool:
        self.log.debug(f"Pairing event: `numeric_comparison` (io_capability: {self.io_capability})")
    ...
    async def get_string(self, max_length: int) -> Optional[str]:
        self.log.debug(f"Pairing event: `pin_code_request` (io_capability: {self.io_capability})")
...
class SecurityService(SecurityServicer):
        ...
        def pairing_config_factory(connection: BumbleConnection) -> PairingConfig:
            return PairingConfig(
                ...
                identity_address_type=(PairingConfig.AddressType.PUBLIC
                                       if connection.self_address.is_public else
                                       config.identity_address_type),
    ...
    async def OnPairing(self, request: AsyncIterator[PairingEventAnswer],
                        context: grpc.ServicerContext) -> AsyncGenerator[PairingEvent, None]:
    ...
    @utils.rpc
    async def WaitSecurity(self, request: WaitSecurityRequest,
                           context: grpc.ServicerContext) -> WaitSecurityResponse:
        connection_handle = int.from_bytes(request.connection.cookie.value, 'big')
...
class SecurityStorageService(SecurityStorageServicer):
    ...
    @utils.rpc
    async def IsBonded(self, request: IsBondedRequest,
    ...
    @utils.rpc
    async def DeleteBond(self, request: DeleteBondRequest,
