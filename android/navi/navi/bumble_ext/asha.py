        ...
        return bytes(
            core.AdvertisingData([
                (
                    ...
                    bytes([self.protocol_version, self.capability]) + self.hisyncid[:4],
                ),
            ]))

    # Handler for audio control commands
    async def _on_audio_control_point_write(self, connection: bumble_device.Connection | None,
                                            value: bytes) -> None:
        del connection
        ...
        if opcode != OpCode.STATUS:
            await self.device.notify_subscribers(self.audio_status_characteristic, force=True)

    # Handler for volume control
    def _on_volume_write(self, connection: bumble_device.Connection | None, value: bytes) -> None:
        del connection
...
class AshaServiceProxy(gatt_client.ProfileServiceProxy):
    ...
    _CHARACTERISTICS = {
        gatt.GATT_ASHA_READ_ONLY_PROPERTIES_CHARACTERISTIC: ('read_only_properties_characteristic'),
        gatt.GATT_ASHA_AUDIO_CONTROL_POINT_CHARACTERISTIC: ('audio_control_point_characteristic'),
        gatt.GATT_ASHA_AUDIO_STATUS_CHARACTERISTIC: ('audio_status_point_characteristic'),
    ...
    def __init__(self, service_proxy: gatt_client.ServiceProxy) -> None:
        ...
        for uuid, attribute_name in self._CHARACTERISTICS.items():
            if not (characteristics := self.service_proxy.get_characteristics_by_uuid(uuid)):
