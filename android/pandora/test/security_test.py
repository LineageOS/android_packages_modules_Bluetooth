...
DEFAULT_SMP_KEY_DISTRIBUTION = (PairingDelegate.KeyDistribution.DISTRIBUTE_ENCRYPTION_KEY |
                                PairingDelegate.KeyDistribution.DISTRIBUTE_IDENTITY_KEY |
                                PairingDelegate.KeyDistribution.DISTRIBUTE_SIGNING_KEY)


async def le_connect_with_rpa_and_encrypt(central: PandoraDevice,
                                          peripheral: PandoraDevice) -> None:
...
class SecurityTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def test_ssp(
        ...
        if self.dut.name == 'android' and connect == 'outgoing_connection' and pair == 'incoming_pairing':
            ...
            raise signals.TestSkip('TODO: Fix rootcanal when both side trigger authentication:\n' +
                                   'Android always trigger auth for outgoing connections.')

        if self.dut.name == 'android' and 'disconnect' in variant:
            raise signals.TestSkip('TODO: Fix AOSP pandora server for this variant:\n' +
        ...
        if self.dut.name == 'android' and pair == 'outgoing_pairing' and ref_role == 'against_central':
            raise signals.TestSkip('TODO: Fix PandoraSecurity server for android:\n' +
                                   'report the encryption state the with the bonding state')

        if self.ref.name == 'android':
            raise signals.TestSkip('TODO: (add bug number) Fix core stack:\n' +
                                   'BOND_BONDED event is triggered before the encryption changed')

        if isinstance(self.ref,
                      BumblePandoraDevice) and ref_io_capability == 'against_default_io_cap':
        ...
        if isinstance(self.ref, BumblePandoraDevice):
            ...
            await self.ref.device.send_command(
                HCI_Write_Default_Link_Policy_Settings_Command(default_link_policy_settings=0x01),
                check_result=True)  # type: ignore
        ...
        async def connect_and_pair() -> Tuple[SecureResponse, WaitSecurityResponse]:
            ...
            if connect == 'incoming_connection':
                ref_dut, dut_ref = await pandora_snippet.connect(initiator=self.ref,
                                                                 acceptor=self.dut)
            else:
                dut_ref, ref_dut = await pandora_snippet.connect(initiator=self.dut,
                                                                 acceptor=self.ref)
        ...
        dut_pairing, ref_pairing = self.dut.aio.security.OnPairing(
        ), self.ref.aio.security.OnPairing()
        ...
        try:
            ...
            if not connect_and_pair_task.done():
                ...
                if dut_ev.method_variant() in ('numeric_comparison', 'just_works'):
                    ...
                    confirm = True
                    if (dut_ev.method_variant() == 'numeric_comparison' and
                            ref_ev.method_variant() == 'numeric_comparison'):
                        confirm = ref_ev.numeric_comparison == dut_ev.numeric_comparison

                    dut_ev_answer = PairingEventAnswer(
                        event=dut_ev, confirm=False if variant == 'reject' else confirm)
            ...
            try:
                ...
                logging.info(
                    f'Pairing result: {secure.result_variant()}/{wait_security.result_variant()}')
                ...
                if shall_pass:
                    ...
                    assert_in(
                        ...
                        ('connection_died', 'pairing_failure', 'authentication_failure',
                         'not_reached'),
                    )
                    assert_in(
