...
class SuiteRunner:
    ...
    def add_path(self, path: pathlib.Path, root: bool = True) -> None:
        if path.is_file():
            ...
            elif not self.test_run_configs and not root and path.name in ('config.yml',
                                                                          'config.yaml'):
    ...
    def is_included(self, cls: base_test.BaseTestClass, test: str) -> bool:
        return not self.test_filters or any(
            filter_match(cls, test, filter) for filter in self.test_filters)
    ...
    def run(self) -> bool:
        ...
        os.environ.setdefault('BUMBLE_SNOOPER',
                              f'btsnoop:file:{self.logs_dir}/{_BUMBLE_BTSNOOP_FMT}')
        ...
        for config in self.test_run_configs:
            ...
            with runner.mobly_logger(
                    console_level=logging.DEBUG if self.logs_verbose else logging.INFO):
