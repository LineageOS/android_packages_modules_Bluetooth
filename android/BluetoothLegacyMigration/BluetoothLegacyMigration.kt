...
class BluetoothLegacyMigration : ContentProvider() {
    ...
    override fun getType(uri: Uri): String {
        val database =
            UriId.values().firstOrNull { it.ordinal == URI_MATCHER.match(uri) }
     ...
     *
     * @param method Action to perform. One of
     *   START_LEGACY_MIGRATION_CALL|FINISH_LEGACY_MIGRATION_CALL
        ...
        private fun putObjectInBundle(bundle: Bundle, key: String, obj: Any?) {
            when (obj) {
                ...
                null -> throw UnsupportedOperationException("null type is not handled")
                    ...
                    throw UnsupportedOperationException(
                        "${obj.javaClass.simpleName}: type is not handled"
                    )
            }
        }
    }
