...
public class BluetoothMediaBrowserService extends MediaBrowserServiceCompat {
        ...
        if (currentState != null) {
            connectingState =
                    new PlaybackStateCompat.Builder(currentState)
                            ...
                            .build();
            service.setPlaybackState(connectingState);
            service.setPlaybackState(currentState);
        } else {
            Log.w(
                    TAG,
                    "onAudioFocusStateChanged(state="
    ...
    private synchronized void setNowPlayingQueue(List<MediaItem> songList) {
