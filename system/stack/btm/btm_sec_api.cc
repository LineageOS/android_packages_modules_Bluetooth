...
public:
  void BTM_Sec_Init() const override { ::BTM_Sec_Init(); }
  void BTM_Sec_Free() const override { ::BTM_Sec_Free(); }

  void BTM_SetPinType(uint8_t pin_type, PinCode pin_code, uint8_t pin_code_len) const override {
    ::BTM_SetPinType(pin_type, pin_code, pin_code_len);
  }

  tBTM_LINK_KEY_TYPE BTM_SecGetDeviceLinkKeyType(const RawAddress& bd_addr) const override {
    return ::BTM_SecGetDeviceLinkKeyType(bd_addr);
  }
  ...
  void BTM_ReadLocalOobData(void) const override { ::BTM_ReadLocalOobData(); }

  bool BTM_PeerSupportsSecureConnections(const RawAddress& bd_addr) const override {
    return ::BTM_PeerSupportsSecureConnections(bd_addr);
  }

  bool BTM_SecRegister(const BtmAppReg& app_reg) const override {
    return ::BTM_SecRegister(app_reg);
  }
  ...
  bool BTM_SecDeleteDevice(const RawAddress& bd_addr) const override {
    return ::BTM_SecDeleteDevice(bd_addr);
  }
  ...
  uint8_t BTM_BleReadSecKeySize(const RawAddress& bd_addr) const override {
    return ::BTM_BleReadSecKeySize(bd_addr);
  }

  void BTM_SecHciDeleteStoredLinkKey(const RawAddress& bd_addr) const override {
    ::BTM_SecHciDeleteStoredLinkKey(bd_addr);
  }

  // other misc APIs
  uint8_t BTM_GetSecurityMode() const override { return ::BTM_GetSecurityMode(); }

  // remote name request related APIs
  const char* BTM_SecReadDevName(const RawAddress& bd_addr) const override {
    return ::BTM_SecReadDevName(bd_addr);
  }
  DEV_CLASS BTM_SecReadDevClass(const RawAddress& bd_addr) const override {
    return ::BTM_SecReadDevClass(bd_addr);
  }

  tBTM_STATUS BTM_SecReportBondLoss(const RawAddress& bd_addr,
                                    tBT_TRANSPORT transport) const override {
    return ::BTM_SecReportBondLoss(bd_addr, transport);
  }

  // BLE related APIs
  const Octet16& BTM_GetDeviceEncRoot() const override { return ::BTM_GetDeviceEncRoot(); }
  const Octet16& BTM_GetDeviceIDRoot() const override { return ::BTM_GetDeviceIDRoot(); }
  const Octet16& BTM_GetDeviceDHK() const override { return ::BTM_GetDeviceDHK(); }
  ...
  void BTM_SecSaveLeKey(const RawAddress& bd_addr, tBTM_LE_KEY_TYPE key_type,
    ...
    ::BTM_SecSaveLeKey(bd_addr, key_type, key, pass_to_application);
  }
  void BTM_BleUpdateSecKeySize(const RawAddress& bd_addr, uint8_t enc_key_size) const override {
    ::BTM_BleUpdateSecKeySize(bd_addr, enc_key_size);
  }
  void BTM_BleResetId(void) const override { ::BTM_BleResetId(); }
};
