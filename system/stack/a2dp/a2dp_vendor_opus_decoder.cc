...
bool a2dp_vendor_opus_decoder_decode_packet(BT_HDR* p_buf) {
  ...
  if (numChannels != 1 && numChannels != 2) {
    LOG_ERROR("Invalid numChannels %d in Opus packet", numChannels);
    return false;
  }

  LOG_INFO("numChannels=%d numFrames=%d frameSize=%d frameLen=%d bufferSize=%d offset=%d",
  ...
  if (ret_val < OPUS_OK) {
    LOG_ERROR("Opus DecodeFrame retry failed with %d, dropping packet", ret_val);
    return false;
  }
