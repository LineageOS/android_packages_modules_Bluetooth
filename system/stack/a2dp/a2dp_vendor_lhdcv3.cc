...
// What this device offers a V3 sink. Stereo only, and the whole rate and depth
// range the encoder codes.
...
// Lays out one Media Codec Capabilities element from |p_ie|.
...
// Reads one Media Codec Capabilities element, from the LOSC octet, into
// |p_ie|. A configuration, unlike a capability, must name exactly one sample
// rate and one bit depth.
...
tA2DP_STATUS A2dpCodecConfigLhdcV3Source::setCodecConfig(const uint8_t* p_peer_codec_info,
  ...
  // Sample rate: the user's choice when both ends allow it, otherwise the best
  // they share.
