...
TEST_F(StackBtmSecWithInitFreeTest, btm_sec_allocate_dev_rec__all) {
  ...
  if (!com::android::bluetooth::flags::use_array_instead_list_in_sec_dev_rec()) {
    for (size_t i = 0; i < kBtmSecMaxDeviceRecords; i++) {
      ASSERT_EQ(i, list_length(::btm_sec_cb.sec_dev_rec_));
  ...
  if (!com::android::bluetooth::flags::use_array_instead_list_in_sec_dev_rec()) {
    for (size_t i = 0; i < kBtmSecMaxDeviceRecords; i++) {
      ASSERT_EQ(kBtmSecMaxDeviceRecords, list_length(::btm_sec_cb.sec_dev_rec_));
      p_devices[i] = btm_sec_allocate_dev_rec(bd_addr);
      ASSERT_NE(nullptr, p_devices[i]);
    }
  } else {
    for (size_t i = 0; i < kBtmSecMaxDeviceRecords; i++) {
      /**
       * Since we are now using btm_sec_cb.device_records_ as static array, so there will be no
...
TEST_F(StackBtmSecWithInitFreeTest, btm_sec_temp_bond_auth_authenticated_temporary) {
  ...
  btm_sec_cb.security_mode_ = BTM_SEC_MODE_SERVICE;
...
protected:
  void SetUp() override {
    ...
    btm_sec_cb.pairing_state_ = BTM_PAIR_STATE_IDLE;     // Ensure not busy
  }
...
TEST_F(StackBtmSecSecurityUpgradeTest, MitmUpgradeNotPossible) {
  ...
  btm_sec_cb.devcb_.loc_io_caps = BtIoCap::NO_INPUT_NO_OUTPUT;
  uint16_t initial_sec_flags = p_device_->sec_rec.sec_flags;
  uint8_t initial_sm4 = p_device_->sm4;
