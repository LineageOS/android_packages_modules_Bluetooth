...
SocketHalImpl::SocketHalImpl() {
  ...
  log::assert_that(death_link == STATUS_OK, "Unable to set the death recipient for the Socket HAL");
}
...
hal::SocketCapabilities SocketHalImpl::GetSocketCapabilities() const {
  if (!IsBound()) {
    return {};
  }
...
bool SocketHalImpl::RegisterCallback(hal::SocketHalCallback const* callback) {
  if (!IsBound()) {
    return false;
  }
...
bool SocketHalImpl::Opened(const hal::SocketContext& context) const {
  if (!IsBound()) {
    return false;
  }
...
void SocketHalImpl::Closed(uint64_t socket_id) const {
  if (!IsBound()) {
    return;
  }
