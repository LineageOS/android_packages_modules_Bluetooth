...
Octet16 c1(const Octet16& k, const Octet16& r, const uint8_t* pres, const uint8_t* preq,
...
inline void aes_cmac(const Octet16& key, const uint8_t* message, uint16_t length, uint16_t tlen,
                     uint8_t* p_signature) {
  Octet16 signature = aes_cmac(key, message, length);

  uint8_t* p_mac = signature.data() + (kOctet16Length - tlen);
