...
const std::unordered_map<std::string, BroadcastConfiguration> kPtsBroadcastConfigs = {
        {"lc3_stereo_48_1_2", lc3_stereo_48_1_2}, {"lc3_stereo_48_2_2", lc3_stereo_48_2_2},
        {"lc3_stereo_48_3_2", lc3_stereo_48_3_2}, {"lc3_stereo_48_4_2", lc3_stereo_48_4_2},
        {"lc3_stereo_24_2_1", lc3_stereo_24_2_1}, {"lc3_mono_16_2_1", lc3_mono_16_2_1},
        {"lc3_stereo_16_2_2", lc3_stereo_16_2_2}, {"lc3_stereo_24_2_2", lc3_stereo_24_2_2},
        {"lc3_mono_16_2_2", lc3_mono_16_2_2},
};
...
BroadcastConfiguration GetBroadcastConfig(
  ...
  if (com_android_bluetooth_flags_leaudio_broadcast_sw_config_quality_logic()) {
    if (subgroup_quality.empty()) {
    ...
    if (options) {
      auto it_config = kPtsBroadcastConfigs.find(*options);
      if (it_config != kPtsBroadcastConfigs.end()) {
        return it_config->second;
      }
      log::error("Invalid PTS configuration: \"{}\"", options->c_str());
      return lc3_stereo_16_2_2;
    }

    auto it_non_high_quality = std::find_if(
            subgroup_quality.begin(), subgroup_quality.end(),
            [](const auto& pair) { return pair.second != bluetooth::le_audio::QUALITY_HIGH; });

    if (it_non_high_quality == subgroup_quality.end()) {
      return lc3_stereo_48_2_1;
    }

    auto it_standard_quality = std::find_if(
            subgroup_quality.begin(), subgroup_quality.end(),
            [](const auto& pair) { return pair.second == bluetooth::le_audio::QUALITY_STANDARD; });

    if (it_standard_quality == subgroup_quality.end()) {
      log::error("Lack of standard quality in subgroups");
    }
