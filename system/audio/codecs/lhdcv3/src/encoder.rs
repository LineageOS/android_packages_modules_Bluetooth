...
/// block is collapsing and those bits buy nothing back. Swept over the corpus.
...
/// frame length against the log of the step over an open corpus. The search
...
///
/// Swept over the corpus: one costs a sixth of the encoder walking a step at
/// a time toward a budget that is octaves away, and anything past five starts
/// overshooting the bracket faster than halving recovers it.
...
fn select_order_over(x: &[i32], rows: &mut Rows, cap: usize, warm_bytes: usize) -> Option<Fixed> {
    ...
    // again, so the search stops there. Carrying it to every order the format
    // can name gives the same answer on the corpus and takes half as long.
    let mut limit = cap;
    for k in 2..cap {
        a[k] = rows.sum(k);
...
/// where the fixed orders beat it. It earns most of a dB on material a fixed
/// order cannot follow and next to nothing on the rest, for a sixth of the
/// encoder, so it is asked for again only after it has lost.
///
/// Swept over the corpus: two costs a further tenth of a dB at the top rate
/// for a fiftieth of the encoder.
/// Blocks the choice between the left channel and the difference is taken
/// from the block before rather than weighed again. It holds from one block
/// to the next nine times in ten, and weighing the one the frame will not
/// carry is a third of the order searches.
///
/// Swept over the corpus: four puts rows more than three dB behind the
/// reference at the top rate for another half second over the corpus.
...
///
/// Swept over the corpus: eight and past start putting rows more than three
/// dB behind the reference for very little more of the encoder.
...
/// with a handful of peaks is not, whatever its mean says. Swept over the
/// corpus against both a wider and a narrower span.
...
/// code on. Swept over the corpus: sitting out none of them is both slower and
/// worse, and sitting out eleven gives up half a dB at the lowest rate.
...
impl Encoder {
    ...
    fn build(
        ...
        // holds from one block to the next nine times in ten, so only the one
        // it carried last time is formed and weighed until the choice is
        // looked at again.
