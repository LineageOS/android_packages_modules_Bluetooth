...
bool btif_storage_get_hearing_aid_prop(const RawAddress& /* address */,
...
bt_status_t btif_storage_add_hid_device_info(
    RawAddress* /* remote_bd_addr */, uint16_t /* attr_mask */,
...
bt_status_t btif_storage_add_remote_device(
    const RawAddress* /* remote_bd_addr */, uint32_t /* num_properties */,
...
bt_status_t btif_storage_get_ble_bonding_key(
    const RawAddress& /* remote_bd_addr */, uint8_t /* key_type */,
...
bt_status_t btif_storage_get_remote_addr_type(
    const RawAddress* /* remote_bd_addr */, tBLE_ADDR_TYPE* /* addr_type */) {
  inc_func_call_count(__func__);
  return BT_STATUS_SUCCESS;
}
...
bt_status_t btif_storage_remove_hid_info(
    const RawAddress& /* remote_bd_addr */) {
  inc_func_call_count(__func__);
  return BT_STATUS_SUCCESS;
}
...
size_t btif_split_uuids_string(const char* /* str */,
...
void btif_storage_set_gatt_cl_db_hash(const RawAddress& /* bd_addr */,
                                      Octet16 /* hash */) {
  inc_func_call_count(__func__);
}
...
void btif_storage_set_gatt_sr_supp_feat(const RawAddress& /* addr */,
                                        uint8_t /* feat */) {
  inc_func_call_count(__func__);
}
