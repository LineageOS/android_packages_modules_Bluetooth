...
void BTA_HhAddDev(const tAclLinkSpec& /* link_spec */,
...
void BTA_HhSendCtrl(uint8_t /* dev_handle */,
                    tBTA_HH_TRANS_CTRL_TYPE /* c_type */) {
  inc_func_call_count(__func__);
}
void BTA_HhSendData(uint8_t /* dev_handle */,
                    const tAclLinkSpec& /* link_spec */, BT_HDR* /* p_data */) {
  inc_func_call_count(__func__);
}
void BTA_HhSetIdle(uint8_t /* dev_handle */, uint16_t /* idle_rate */) {
  inc_func_call_count(__func__);
}
void BTA_HhSetProtoMode(uint8_t /* dev_handle */,
                        tBTA_HH_PROTO_MODE /* p_type */) {
  inc_func_call_count(__func__);
}
