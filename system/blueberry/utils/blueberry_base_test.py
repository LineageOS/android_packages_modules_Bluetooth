  ...
  """
    ...
    """
        ...
        for test_name in selected_test_names:
            ...
            test_arg_sets = [('%s_%s_of_%s' % (test_name, i + 1, test_iterations), test_method)
                             for i in range(test_iterations)]
            # pylint: disable=cell-var-from-loop
            self.generate_tests(test_logic=lambda _, test: test(self),
    ...
    def setup_class(self):
        ...
        self._upload_test_report = int(self.user_params.get('upload_test_report', 0))
        # Inits Spanner Utils if need to upload the test reports to Spanner.
        if self._upload_test_report:
            self._init_spanner_utils()
        self.capture_bugreport_on_fail = int(self.user_params.get('capture_bugreport_on_fail', 0))
        self.ignore_device_setup_failures = int(
            self.user_params.get('ignore_device_setup_failures', 0))
        self.enable_bluetooth_verbose_logging = int(
            self.user_params.get('enable_bluetooth_verbose_logging', 0))
        self.enable_hci_snoop_logging = int(self.user_params.get('enable_hci_snoop_logging', 0))
        self.increase_logger_buffers = int(self.user_params.get('increase_logger_buffers', 0))
        self.enable_all_bluetooth_logging = int(
            self.user_params.get('enable_all_bluetooth_logging', 0))

        # base test should include the test between primary device with Bluetooth
        # peripheral device.
        self.android_devices = self.register_controller(android_device, required=False)

        # In the case of no android_device assigned, at least 2 derived_bt_device
        # is required.
        if self.android_devices is None:
            self.derived_bt_devices = self.register_controller(module=derived_bt_device,
                                                               min_number=2)
        else:
            self.derived_bt_devices = self.register_controller(module=derived_bt_device,
                                                               required=False)
        ...
        for device in self.android_devices:
            ...
            if (self.enable_bluetooth_verbose_logging or self.enable_all_bluetooth_logging):
        ...
        for num_devices in range(1, len(self.android_devices)):
            for decorator in self.target_decorators:
                self.android_devices[num_devices] = android_bluetooth_client_decorator.decorate(
        ...
        if self.capture_bugreport_on_fail:
            ...
            for d in self.derived_bt_devices:
                if hasattr(d, 'take_bug_report'):
                    devices = devices + [d]
            android_device.take_bug_reports(devices,
                                            record.test_name,
                                            record.begin_time,
    ...
    def _init_spanner_utils(self) -> None:
        ...
        spanner_utils_module = importlib.import_module('blueberry.utils.spanner_utils')
        self._spanner_utils = spanner_utils_module.SpannerUtils(
            test_class_name=self.__class__.__name__,
            mh_sponge_link=self.user_params['mh_sponge_link'])

    def _upload_test_report_to_spanner(self, result: records.TestResultEnums) -> None:
        """Uploads the test report to Spanner.

    Args:
      result: Result of this test.
    """
        self._spanner_utils.create_test_report_proto(current_test_info=self.current_test_info,
    ...
    def record_test_metric(self, metric_name: str, metric_value: Union[int, float]) -> None:
    ...
    """
        if not self._upload_test_report:
            return
        self._test_metrics.append(self._spanner_utils.create_metric_proto(
