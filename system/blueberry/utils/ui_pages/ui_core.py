...
class Context(abc.ABC):
  ...
  """
    ...
    """
        output = self.ad.adb.shell('dumpsys activity | grep -E mFocusedApp').decode()
        if any([
                not output, 'not found' in output, "Can't find" in output, 'mFocusedApp=null'
                in output
        ]):
            self.log.warning('Fail to obtain the current app activity with output: %s', output)
            return ''
    ...
    @retry.logged_retry_on_exception(retry_value=(adb.Error, errors.ContextError),
    ...
    """
        ...
        self.ad.adb.shell(f"test -f {dump_xml_path} && rm {dump_xml_path} || echo 'no dump xml'")
    ...
    """
        ...
        raise errors.ContextError(
            self, f'Fail to reach page(s): {page_class_list} (current page={page})!')
    ...
    def get_page(self, wait_sec: int = 1, xml_path: Optional[str] = None) -> UIPage:
        if self.safe_get:
    ...
    def safe_get_page(self, wait_sec: int = 1, xml_path: Optional[str] = None) -> UIPage:
    ...
    """
        ...
        for page_class in self.known_pages:
            page_obj = page_class.from_xml(self, ui_xml, clickable_nodes, enabled_nodes, all_nodes)
        ...
        raise errors.UnknownPageError(ui_xml, clickable_nodes, enabled_nodes, all_nodes)
...
class UIPage:
  ...
  """
    ...
    def __init__(self, ctx: Context, ui_xml: Optional[minidom.Document],
                 clickable_nodes: List[ui_node.UINode], enabled_nodes: List[ui_node.UINode],
    ...
    @classmethod
    def from_xml(cls, ctx: Context, ui_xml: minidom.Document, clickable_nodes: List[ui_node.UINode],
                 enabled_nodes: List[ui_node.UINode],
    ...
    """
        if cls.PAGE_RID is not None:
            ...
            for node in enabled_nodes + clickable_nodes:
                if all([node.resource_id == 'android:id/title', node.text == cls.PAGE_TITLE]):
                    return cls(ctx, ui_xml, clickable_nodes, enabled_nodes, all_nodes)
        else:
    ...
    def _get_node_search_space(self, from_all: bool = False) -> Iterable[ui_node.UINode]:
    ...
    def get_node_by_content_desc(self, content_desc: str, from_all: bool = False) -> OptUINode:
    ...
    def _yield_node_by_rid(self, rid: str, from_all: bool = False) -> NodeGenerator:
        """Generates node with desired resource id."""
        for node in self._get_node_search_space(from_all):
            if ('resource-id' in node.attributes and node.attributes['resource-id'].value == rid):
                yield node

    def get_all_nodes_by_rid(self, rid: str, from_all: bool = False) -> List[ui_node.UINode]:
    ...
    def get_node_by_rid(self, rid: str, from_all: bool = False) -> Optional[ui_node.UINode]:
    ...
    def get_node_by_attrs(self, attrs: Dict[str, Any], from_all: bool = False) -> OptUINode:
    ...
    """
        for _ in range(swipes):
            self.ctx.ad.adb.shell(
                shlex.split(f'input swipe {start_x} {start_y} {end_x} {end_y} {duration_ms}'))
    ...
    """
        ...
        return self.swipe(width * x_start,
                          height * 0.5,
                          width * x_end,
                          height * 0.5,
    ...
    @utils.dr_wakeup_before_op
    def click(self, node: ui_node.UINode, do_get_page: bool = True) -> Optional[UIPage]:
    ...
    def click_node_by_rid(self, node_rid: str, do_get_page: bool = True) -> UIPage:
    ...
    def click_node_by_content_desc(self, text: str, do_get_page: bool = True) -> UIPage:
    ...
    """
        ...
        if node is None:
            raise errors.UIError(f'Fail to find the node with content description={text}')

        return self.click(node, do_get_page)

    def click_node_by_class(self, class_value: str, do_get_page: bool = True) -> UIPage:
