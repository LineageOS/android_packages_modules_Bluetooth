...
class FitbitAppDecorator:
  ...
  """
    ...
    def __init__(self, ad: android_device.AndroidDevice):  # pylint: disable=super-init-not-called
        ...
        self.ui_context = fitbit_companion.get_context(self._ad, do_go_home=False, safe_get=True)
        ...
        if not apk_utils.is_apk_installed(self._ad, _FITBIT_PACKAGE_NAME):
            ...
            self.ui_context.log.info('Login Fitbit App with account=%s...', fitbit_app_account)
            self.ui_context.page.login(fitbit_app_account,
    ...
    """
        log = FitbitCompanionAppLoggerAdapter(logging.getLogger(), {'tag': mac_address})
        ...
        if target_device_mac_address != mac_address:
            raise ValueError((f'Target BT device has MAC address={target_device_mac_address}',
        ...
        self.ui_context.regr_page_call(pairing_pages.UpdateDevicePage, 'update_later')
        ...
        self.ui_context.expect_page(pairing_pages.ChooseTrackerPage,
        ...
        while True:
            ...
            pair_retry += 1
            if pair_retry >= _MAX_PAIRING_RETRIES:
                raise signals.TestError(f'Failed in pairing process after {pair_retry} tries!')
        ...
        self.ui_context.expect_pages(
            [pairing_pages.PremiumPage, other_pages.PurchaseFail, account_pages.AccountPage])
        ...
        log.info('Pairing and connection with %s(%s) is all done!', fitbit_prod_name, mac_address)
