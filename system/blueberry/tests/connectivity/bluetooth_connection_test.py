  ...
  """
    ...
    """
        with asserts.assert_not_raises(signals.TestError):
            self.primary_device.wait_for_a2dp_connection_state(self.mac_address, connected)
            self.primary_device.wait_for_hsp_connection_state(self.mac_address, connected)

    def test_disconnect_and_connect(self):
        """Test for DUT disconnecting and then connecting to the remote device."""
        self.primary_device.log.info('Disconnecting the device "%s"...' % self.mac_address)
        ...
        time.sleep(CONNECTION_STATE_CHANGE_SLEEP_SEC)
        self.primary_device.log.info('Connecting the device "%s"...' % self.mac_address)
    ...
    """
        self.primary_device.log.info('The connected device "%s" is being powered off...' %
                                     self.mac_address)
        ...
        self.primary_device.log.info('The connected device "%s" is being powered on...' %
                                     self.mac_address)
        self.primary_device.wait_for_connection_success(self.mac_address)
        self.assert_a2dp_and_hsp_connection_state(connected=True)
