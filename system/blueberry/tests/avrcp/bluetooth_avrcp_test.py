...
class BluetoothAvrcpTest(blueberry_base_test.BlueberryBaseTest):
  ...
  """
    ...
    def setup_class(self):
        ...
        try:
            ...
            if 'No such file or directory' in str(error):
                raise signals.TestError('No directory "%s" found on the primary device "%s".' %
        ...
        # commands can work.
        self.pri_device.sl4a.bluetoothMediaHandleMediaCommandOnPhone(bt_constants.CMD_MEDIA_PLAY)
        ...
        self.pri_device.sl4a.bluetoothMediaHandleMediaCommandOnPhone(bt_constants.CMD_MEDIA_PAUSE)
    ...
    """
        ...
        self.derived_bt_device.verify_playback_state_changed(
            ...
            exception=signals.TestError(
                ...
                '"%s" != "%s"' %
                (self.derived_bt_device.get_current_playback_state(), expected_state)))
        ...
        self.derived_bt_device.verify_current_track_changed(
            ...
            exception=signals.TestError(
                ...
                '"%s" != "%s"' % (self.derived_bt_device.get_current_track_info(), expected_track)))
    ...
    """
        ...
        if test_command not in [bt_constants.CMD_MEDIA_PLAY, bt_constants.CMD_MEDIA_PAUSE]:
            raise signals.TestError(
                'Command "%s" is invalid. The test command should be "%s" or "%s".' %
                (test_command, bt_constants.CMD_MEDIA_PLAY, bt_constants.CMD_MEDIA_PAUSE))

        # Make sure the playback state is playing if testing the command "pause".
        if (self.pri_device.get_current_playback_state() != bt_constants.STATE_PLAYING and
        ...
        self.pri_device.log.info('Current playback state: %s' %
        ...
        for device in device_check_list:
            device.verify_playback_state_changed(
                ...
                exception=signals.TestFailure(
                    ...
                    'Current state: %s' %
                    (expected_state, device.serial, device.get_current_playback_state())))
    ...
    """
        # Checks if the test command is valid.
        if test_command not in [bt_constants.CMD_MEDIA_SKIP_NEXT, bt_constants.CMD_MEDIA_SKIP_PREV]:
            raise signals.TestError(
                'Command "%s" is invalid. The test command should be "%s" or "%s".' %
                (test_command, bt_constants.CMD_MEDIA_SKIP_NEXT, bt_constants.CMD_MEDIA_SKIP_PREV))

        # Make sure the track index is not 0 if testing the command "skipPrev".
        if (self.tracks.index(self.pri_device.get_current_track_info()) == 0 and
    ...
    def test_media_pause(self):
        ...
        self.execute_media_play_pause_test_logic(command_sender=self.derived_bt_device,
                                                 test_command=bt_constants.CMD_MEDIA_PAUSE)
    ...
    def test_media_skip_prev(self):
        ...
        self.execute_skip_next_prev_test_logic(command_sender=self.derived_bt_device,
                                               test_command=bt_constants.CMD_MEDIA_SKIP_PREV)
    ...
    """
        ...
        self.execute_media_play_pause_test_logic(command_sender=self.pri_device,
                                                 test_command=bt_constants.CMD_MEDIA_PAUSE)
    ...
    """
        ...
        self.execute_skip_next_prev_test_logic(command_sender=self.pri_device,
                                               test_command=bt_constants.CMD_MEDIA_SKIP_PREV)
