...
class BluetoothPanTest(blueberry_base_test.BlueberryBaseTest):
  ...
  """
    ...
    def setup_class(self):
        ...
        for device in self.android_devices:
            ...
            if device.sl4a.telephonyGetSimState() != SIM_STATE_READY:
                raise signals.TestError('SIM card is not ready on Device "%s".' % device.serial)
    ...
    def wait_for_nap_service_connection(self, device, connected_mac_addr, state_connected=True):
    ...
    """
        ...
        def is_device_connected():
            ...
            connected_devices = (device.sl4a.bluetoothPanGetConnectedDevices())
        ...
        bt_test_utils.wait_until(
            ...
            exception=signals.TestFailure(
                ...
                ('established' if state_connected else 'terminated', CONNECTION_TIMEOUT_SECS)))

    def initiate_nap_service_connection(self, initiator_device, connected_mac_addr):
    ...
    """
        ...
        for _ in range(self.pan_connect_attempts):
            ...
            try:
                ...
                self.wait_for_nap_service_connection(device=initiator_device,
                ...
                if count == self.pan_connect_attempts:
                    raise signals.TestFailure(
                        ...
                        'after retried %d times.' % self.pan_connect_attempts)

    def terminate_nap_service_connection(self, initiator_device, connected_mac_addr):
    ...
    """
        ...
        try:
            ...
            panu_device.log.info('Initiate a connection to NAP device "%s".' % nap_device.serial)
            self.initiate_nap_service_connection(initiator_device=panu_device,
                                                 connected_mac_addr=nap_device.mac_address)

            # Check if PANU device can access internet via NAP service connection.
            self.verify_internet(allow_access=True,
                                 device=panu_device,
                                 exception=signals.TestFailure(
    ...
    """
        device.log.info('Verify that internet %s be used.' % ('can' if allow_access else 'can not'))
        ...
        bt_test_utils.wait_until(timeout_sec=PING_TIMEOUT_SEC,
    ...
    """
        with self.establish_nap_service_connection(nap_device=self.secondary_device,
            ...
            self.verify_internet(
                ...
                exception=signals.TestFailure(
                    ...
                    'terminated NAP service connection.' % self.primary_device.serial))
    ...
    """
        with self.establish_nap_service_connection(nap_device=self.primary_device,
            ...
            self.verify_internet(
                ...
                exception=signals.TestFailure(
                    'PANU device "%s" can still access internet when it had '
                    'terminated NAP service connection.' % self.secondary_device.serial))
